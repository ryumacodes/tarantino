...
impl FFmpegManager {
    ...
    pub async fn execute_operation(
        ...
        println!(
            "FFmpegManager: Queuing operation {} with priority {:?}",
        ...
        if can_execute {
            ...
            self.queue_operation(operation_id, operation, priority)
                .await
        }
    }
    ...
    async fn execute_immediately(
        ...
        println!(
            "FFmpegManager: Executing operation {} immediately",
            operation_id
        );
        ...
        let result = self
            .execute_operation_internal(operation_id, &operation, false)
            .await?;
            ...
            if operations::should_retry_with_software(error_msg) {
                ...
                if matches!(operation, FFmpegOperation::Thumbnail { .. }) {
                    let retry_result = self
                        .execute_with_fallback_levels(operation_id, &operation)
                        .await?;

                    // Clean up and update stats
                    self.cleanup_operation(operation_id, &retry_result, start_time)
                        .await;
        ...
        // Clean up and update stats
        self.cleanup_operation(operation_id, &result, start_time)
            .await;
        self.process_queue().await;
    ...
    async fn execute_with_fallback_levels(
        ...
        let mut result = self
            .execute_operation_with_fallback_level(operation_id, operation, 1)
            .await?;
        ...
        if matches!(result, OperationResult::Error(_)) {
            ...
            result = self
                .execute_operation_with_fallback_level(operation_id, operation, 2)
                .await?;
        }
        ...
        if matches!(result, OperationResult::Error(_)) {
            ...
            result = self
                .execute_operation_with_fallback_level(operation_id, operation, 3)
                .await?;
        }
        ...
        match &result {
            OperationResult::Success(_) => {
                println!(
                    "[FFMPEG FALLBACK] ✓ Successfully recovered thumbnail using software fallback"
                );
            }
            OperationResult::Error(e) => {
                println!(
                    "[FFMPEG FALLBACK] ✗ All fallback levels failed. Video may be too corrupted."
                );
                println!(
                    "[FFMPEG FALLBACK] Final error: {}",
    ...
    async fn execute_operation_internal(
        ...
        println!(
            "FFmpegManager: Spawned process with PID {:?} for operation {} (hwaccel: {})",
            child.id(),
            operation_id,
            !disable_hwaccel
        );
    ...
    async fn execute_operation_with_fallback_level(
        ...
        println!(
            "FFmpegManager: Spawned process with PID {:?} for operation {} (fallback level {})",
            child.id(),
            operation_id,
            fallback_level
        );
    ...
    async fn queue_operation(
        ...
        println!(
            "FFmpegManager: Queuing operation {} for later execution",
            operation_id
        );
    ...
    async fn cleanup_operation(
        ...
        if total_ops > 0 {
            stats.average_operation_time_ms =
                (stats.average_operation_time_ms * (total_ops - 1) as f64 + duration_ms)
                    / total_ops as f64;
        }

        println!(
            "FFmpegManager: Cleaned up operation {} ({:?}) in {:.2}ms",
            operation_id,
            result,
