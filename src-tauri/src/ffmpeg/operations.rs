...
pub fn build_command(operation: &FFmpegOperation) -> Result<AsyncCommand> {
    let mut cmd = match operation {
        FFmpegOperation::Thumbnail {
            ...
            cmd.args([
                "-vf",
                &format!("scale={}:-1:flags=fast_bilinear,format=yuvj420p", width),
                "-vframes",
                "1",
                "-c:v",
                "mjpeg",
...
pub fn build_command_software_with_fallback_level(
    ...
    let mut cmd = match operation {
        FFmpegOperation::Thumbnail {
            ...
            cmd.args(["-err_detect", "ignore_err", "-skip_frame", "nokey"]);
            ...
            match fallback_level {
                1 => {
                    ...
                    cmd.args(["-ss", &format!("{:.3}", time_offset)]);
                    println!(
                        "[FFMPEG FALLBACK L1] Software decoder with seeking (time offset: {:.3}s)",
                        time_offset
                    );
                }
                2 => {
                ...
                3 => {
                    ...
                    println!(
                        "[FFMPEG FALLBACK L3] Software decoder from middle (time offset: {:.3}s)",
                        middle_time
                    );
                }
                _ => {
                    println!(
                        "[FFMPEG FALLBACK] Unknown level {}, using L1",
                        fallback_level
                    );
                    cmd.args(["-ss", &format!("{:.3}", time_offset)]);
                }
...
fn build_filter_chain(operations: &[VideoOperation]) -> String {
    let filters: Vec<String> = operations
        .iter()
        .map(|op| match op {
...
pub fn validate_thumbnail_output(
    ...
    stderr: &[u8],
) -> super::types::OperationResult {
    use super::types::OperationResult;

    if !output_path.exists() {
        let stderr_str = String::from_utf8_lossy(stderr);
        println!(
            "FFmpegManager: Operation {} reported success but output file not created. stderr: {}",
            operation_id, stderr_str
        );
        return OperationResult::Error(format!(
            "Output file not created. FFmpeg stderr: {}",
            stderr_str
        ));
    }
    ...
    if let Ok(metadata) = std::fs::metadata(output_path) {
        if metadata.len() < 100 {
            println!(
                "FFmpegManager: Operation {} created suspiciously small file ({} bytes)",
                operation_id,
                metadata.len()
            );
            return OperationResult::Error(
                "Output file too small (possibly corrupted)".to_string(),
            );
        }
    }
