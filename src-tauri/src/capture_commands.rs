...
pub async fn record_start_native(
    ...
    let source_id = if let Some(id) = config.target.id.map(|v| v as u64) {
        ...
        if let Some(src) = sources
            .iter()
            .find(|s| s.source_type == CaptureSourceType::Display && s.id.to_string() == sel)
        {
            src.id
        } else {
            sources
    ...
    let _ = crate::commands::tray::update_main_tray_timer_cmd(app.clone(), "00:00:00".to_string())
        .await;

    // Timer is managed by the main record_start function to avoid duplicate timers

    // Emit recording started event
    app.emit("recording:started", &response)
        .map_err(|e| e.to_string())?;

    println!(
        "Native recording started successfully: session {}",
        response.session_id
    );
    Ok(response)
}
...
#[tauri::command]
pub async fn record_toggle_mic(session_id: String, muted: bool) -> Result<(), String> {
    // Not implemented yet for native backend
    println!(
        "record_toggle_mic not yet implemented for native backend (session_id={}, muted={})",
        session_id, muted
    );
    Ok(())
}
...
#[tauri::command]
pub async fn record_toggle_system_audio(session_id: String, muted: bool) -> Result<(), String> {
    // Not implemented yet for native backend
...
#[tauri::command]
pub async fn record_toggle_camera(session_id: String, muted: bool) -> Result<(), String> {
    // Not implemented yet for native backend
    println!(
        "record_toggle_camera not yet implemented for native backend (session_id={}, muted={})",
        session_id, muted
    );
    Ok(())
}
...
pub async fn record_add_break(
    ...
    // Not implemented yet for native backend
    println!(
        "record_add_break not yet implemented for native backend (session_id={})",
        session_id
    );
    // Emit placeholder event for UI continuity
    let response = AddBreakResponse {
        clip_id: uuid::Uuid::new_v4().to_string(),
    };
    app.emit("recording:break-added", &response)
        .map_err(|e| e.to_string())?;
    Ok(response)
}
...
pub async fn record_stop_native(
    ...
    app.emit("recording:stopped", &response)
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn project_open(project_path: String, app: AppHandle) -> Result<Project, String> {
...
#[tauri::command]
pub async fn editor_play(start_time_ms: u64, app: AppHandle) -> Result<(), String> {
    // Emit play command to editor
    app.emit("editor:play", start_time_ms)
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn editor_pause(app: AppHandle) -> Result<(), String> {
...
#[tauri::command]
pub async fn editor_seek(time_ms: u64, app: AppHandle) -> Result<(), String> {
    // Emit seek command to editor
    app.emit("editor:seek", time_ms)
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn editor_add_zoom(segment: ZoomSegment, app: AppHandle) -> Result<String, String> {
    // Emit add zoom command to editor
    app.emit("editor:add-zoom", &segment)
        .map_err(|e| e.to_string())?;
...
/// Delete zoom segment
#[tauri::command]
pub async fn editor_delete_zoom(id: String, app: AppHandle) -> Result<(), String> {
    // Emit delete zoom command to editor
    app.emit("editor:delete-zoom", id.clone())
        .map_err(|e| e.to_string())?;
...
pub async fn export_run(
    ...
    fps: Option<u32>,
    #[allow(unused_variables)] base_resolution: Option<Resolution>,
    #[allow(unused_variables)] quality: Option<f64>, // 0.0 - 1.0
    app: AppHandle,
...
fn _legacy_spawn_tray_timer(app: AppHandle, started_at: i64) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(_) =
                crate::commands::tray::update_main_tray_timer_cmd(app.clone(), time_str).await
            {
...
async fn open_editor_with_project(app: &AppHandle, project_path: &str) -> Result<(), String> {
    ...
    editor_window
        ...
        .map_err(|e| e.to_string())?;
    editor_window
        .set_min_size(Some(Size::Logical(LogicalSize::new(1100.0, 720.0))))
        .ok();
