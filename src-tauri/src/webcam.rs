...
extern "C" {
    ...
    fn avc_hide_preview(session_ptr: *mut c_void);
    fn avc_show_preview(session_ptr: *mut c_void);
    fn avc_set_preview_shape(session_ptr: *mut c_void, shape: *const c_char);
...
#[cfg(target_os = "macos")]
pub fn ensure_camera_permission() -> Result<(), String> {
    let has_permission = unsafe { avc_check_camera_permission() };
    if has_permission {
        return Ok(());
    }

    println!("[Webcam] Requesting native macOS camera permission before WebView capture...");
    let granted = unsafe { avc_request_camera_permission() };
    if granted {
        Ok(())
    } else {
        Err("Camera permission denied. Enable Tarantino in System Settings > Privacy & Security > Camera.".to_string())
    }
}
...
impl WebcamCapture {
    ...
    pub fn start(device_id: Option<&str>, shape: &str, fps: u32) -> Result<Self, String> {
        let has_permission = unsafe { avc_check_camera_permission() };
        if !has_permission {
            ...
            if !granted {
                ...
                return Err("Camera permission denied. Enable Tarantino in System Settings > Privacy & Security > Camera.".to_string());
            }
        }
        ...
        let shape_cstr = std::ffi::CString::new(shape).ok();
        let shape_ptr = shape_cstr
            .as_ref()
            .map(|c| c.as_ptr())
            .unwrap_or(std::ptr::null());
        ...
        if session_ptr.is_null() {
            ...
            return Err(
                "Failed to start camera capture. Check that no other app is using the camera."
                    .to_string(),
            );
        }
    ...
    pub fn hide_preview(&mut self) {
        if !self.session_ptr.is_null() {
            unsafe { avc_hide_preview(self.session_ptr) };
        }
    }

    pub fn show_preview(&mut self) {
        if !self.session_ptr.is_null() {
            unsafe { avc_show_preview(self.session_ptr) };
        }
    }

    pub fn set_shape(&mut self, shape: &str) {
        if !self.session_ptr.is_null() {
            if let Ok(shape_cstr) = std::ffi::CString::new(shape) {
                unsafe { avc_set_preview_shape(self.session_ptr, shape_cstr.as_ptr()) };
            }
        }
    }
    ...
    tokio::task::spawn_blocking(move || {
        ...
        loop {
            ...
            if encoder.is_none() {
                ...
                let mut enc =
                    Encoder::new(config, &out_str).map_err(|e| format!("Encoder init: {}", e))?;
        ...
        if let Some(enc) = encoder.as_ref() {
            if let Some(mux) = muxer.as_mut() {
                while let Some(encoded) = enc.try_receive_frame() {
                    let duration_ms = prev_pts_us
                        .map(|prev| ((encoded.timestamp_us.saturating_sub(prev)) / 1000) as u32)
                        .unwrap_or(1000 / fps);
                    prev_pts_us = Some(encoded.timestamp_us);
                    let _ = mux.write_frame(&encoded, duration_ms.max(1));
                }
