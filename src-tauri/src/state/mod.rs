...
pub struct UnifiedAppState {
    ...
    /// Coalesces native source refreshes so UI clicks never fan out SCK calls.
    pub source_refreshing: AtomicBool,
...
impl UnifiedAppState {
    ...
    pub fn new() -> Result<Self> {
        Ok(Self {
            ...
            source_refreshing: AtomicBool::new(false),
    ...
    pub async fn set_capture_mode(&self, mode: crate::CaptureMode) -> Result<()> {
        let mut app = self.app.write();
        app.capture_mode = match mode {
            crate::CaptureMode::Desktop => {
                app.selected_window_id = None;
                app.selected_device_id = None;
                "desktop".to_string()
            }
            crate::CaptureMode::Window => {
                app.selected_device_id = None;
                "window".to_string()
            }
            crate::CaptureMode::Device => {
                app.selected_window_id = None;
                "device".to_string()
            }
        };
        println!("Capture mode updated: {}", app.capture_mode);
        Ok(())
    }
    ...
    pub async fn select_display(&self, id: String) -> Result<()> {
        ...
        if app.displays.iter().any(|d| d.id == id) {
            app.capture_mode = "desktop".to_string();
    ...
    pub async fn select_window(&self, id: String) -> Result<()> {
        let mut app = self.app.write();
        if app.windows.iter().any(|window| window.id == id) {
            app.capture_mode = "window".to_string();
            app.selected_window_id = Some(id);
            app.selected_device_id = None;
            println!("Selected window updated");
        } else {
            anyhow::bail!(
                "Selected window is not available yet. Refresh the window list and pick it again."
            );
        }
        Ok(())
    }

    pub async fn select_area(&self, _x: f64, _y: f64, _width: f64, _height: f64) -> Result<()> {
        // TODO: Implement area selection
        Ok(())
    }

    pub async fn select_device(&self, id: String) -> Result<()> {
        let mut app = self.app.write();
        app.capture_mode = "device".to_string();
        app.selected_device_id = Some(id);
        app.selected_window_id = None;
        Ok(())
    }
    ...
    pub async fn set_camera_input(
        ...
        *self.camera_device_id.lock() = device_id;
        {
            let mut app = self.app.write();
            app.webcam_config.enabled = enabled;
    ...
    pub async fn set_mic_input(&self, enabled: bool, device_id: Option<String>) -> Result<()> {
        let mut app = self.app.write();
        app.microphone_config.enabled = enabled;
        app.microphone_config.device_id = device_id;
        Ok(())
    }

    pub async fn set_system_audio(&self, enabled: bool, source_id: Option<String>) -> Result<()> {
        let mut app = self.app.write();
        app.system_audio_config.enabled = enabled;
        app.system_audio_config.source_id = source_id;
        Ok(())
    }
