...
impl AppStateContainer {
    ...
    pub fn update_windows(&self, windows: Vec<Window>) {
        ...
        if let Some(selected) = &state.selected_window_id {
            if !state.windows.iter().any(|window| &window.id == selected) {
...
mod tests {
    ...
    fn test_app_state_serialization() {
        ...
        assert_eq!(
            state.settings.default_quality,
            deserialized.settings.default_quality
        );
        assert_eq!(
