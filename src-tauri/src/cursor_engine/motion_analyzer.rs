...
use super::{CursorEngineConfig, CursorEvent, EnhancedCursorEvent};
...
impl MotionAnalyzer {
    ...
    pub fn get_motion_stats(&self) -> MotionStats {
        ...
        let accelerations: Vec<f64> = self
            .position_history
            .iter()
        ...
        let velocity_variance = velocities
            .iter()
...
impl StabilityAnalyzer {
    ...
    fn calculate_stability_score(&self) -> f32 {
        ...
        let positions: Vec<(f64, f64)> = self.stable_points.iter().map(|p| (p.x, p.y)).collect();
