...
use super::{CursorEngineConfig, EnhancedCursorEvent, ZoomEasing, ZoomReason, ZoomRecommendation};
mod smoothing;
mod types;

use smoothing::FocusSmoother;
pub use smoothing::ZoomState;
use types::{ActiveZoom, FocusStrategy, ZoomAction, ZoomConditions, ZoomRule};
...
impl ZoomCalculator {
    ...
    pub fn get_zoom_state(&self, current_time_ms: u64) -> Option<ZoomState> {
        ...
        let active_zoom = self
            .active_zooms
            .iter()
            .filter(|zoom| {
                current_time_ms >= zoom.start_time_ms && current_time_ms <= zoom.end_time_ms
            })
        ...
        match strategy {
            ...
            FocusStrategy::PatternOptimal => {
                ...
                if event.is_reading {
                    ...
                    let focus_x =
                        ((event.x + reading_offset) / self.config.screen_width as f64) as f32;
                    Ok((
    ...
    fn cleanup_expired_zooms(&mut self, current_time_ms: u64) {
        self.active_zooms
            .retain(|zoom| zoom.end_time_ms > current_time_ms);
    }
        ...
        self.active_zooms.iter().any(|zoom| {
            ...
            if time_overlap {
                let focus_distance = ((recommendation.focus_x - zoom.focus_x).powi(2)
                    + (recommendation.focus_y - zoom.focus_y).powi(2))
                .sqrt();
    ...
    fn update_zoom_stats(&mut self, recommendation: &ZoomRecommendation) {
        ...
        *self
        ...
        self.zoom_stats.avg_zoom_factor = (self.zoom_stats.avg_zoom_factor * (total_zooms - 1.0)
            + recommendation.zoom_factor)
            / total_zooms;

        let duration = recommendation.end_time_ms - recommendation.start_time_ms;
        self.zoom_stats.avg_zoom_duration_ms = ((self.zoom_stats.avg_zoom_duration_ms as f32
            * (total_zooms - 1.0))
            + duration as f32) as u64
            / total_zooms as u64;
    }
}
