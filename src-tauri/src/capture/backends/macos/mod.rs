...
static SCK_SHAREABLE_LOCK: std::sync::LazyLock<Mutex<()>> =
...
impl ScreenCaptureKitBackend {
    ...
    pub fn enumerate_windows_only() -> Result<Vec<CaptureSourceInfo>> {
        let _guard = SCK_SHAREABLE_LOCK.lock().unwrap();
        let mut windows = sck_wrapper::get_shareable_windows()?
            .into_iter()
            .map(Self::convert_window_info)
            .collect::<Vec<_>>();
        windows.sort_by(|a, b| a.id.cmp(&b.id));
        Ok(windows)
    }

    pub fn enumerate_displays_only() -> Result<Vec<CaptureSourceInfo>> {
        let _guard = SCK_SHAREABLE_LOCK.lock().unwrap();
        let mut displays = sck_wrapper::get_shareable_displays()?
            .into_iter()
            .map(Self::convert_display_info)
            .collect::<Vec<_>>();
        displays.sort_by(|a, b| match (a.is_primary, b.is_primary) {
...
impl NativeCaptureBackend for ScreenCaptureKitBackend {
    ...
    async fn enumerate_sources(&self) -> Result<Vec<CaptureSourceInfo>> {
        ...
        sources.sort_by(|a, b| match (a.is_primary, b.is_primary) {
            (true, false) => std::cmp::Ordering::Less,
            (false, true) => std::cmp::Ordering::Greater,
            _ => a.id.cmp(&b.id),
        });
    ...
    async fn start_capture(&mut self, config: CaptureConfig) -> Result<CaptureSessionHandle> {
        ...
        *self.audio_sender.lock().unwrap() = Some(audio_sender.clone());
        ...
        // Keep record start off the source-discovery path. The native wrapper
        // validates the selected id while creating the SCContentFilter.
        let source_info = CaptureSourceInfo {
            id: config.source_id,
            name: match config.source_type {
                CaptureSourceType::Display => format!("Display {}", config.source_id),
                CaptureSourceType::Window => format!("Window {}", config.source_id),
            },
        ...
        let make_frame_callback = |sender: broadcast::Sender<CapturedFrame>| {
            move |frame: SCKFrameData| {
                use std::ffi::CStr;
                ...
                let data_slice = unsafe { std::slice::from_raw_parts(frame.data, frame.data_len) };
                ...
                let _ = sender.send(captured_frame);
            }
        };

        let make_audio_callback = |sender: broadcast::Sender<CapturedAudio>| {
            move |audio: SCKAudioData| {
                // Validate pointer and size before creating slice
                ...
                if audio.data_len > isize::MAX as usize {
                    eprintln!(
                        "Warning: Audio data length exceeds isize::MAX, skipping audio sample"
                    );
                    return;
                }

                // Convert C audio data to Rust types
                let data_slice = unsafe { std::slice::from_raw_parts(audio.data, audio.data_len) };
                ...
                let _ = sender.send(captured_audio);
            }
        };
        ...
        let _guard = SCK_SHAREABLE_LOCK.lock().unwrap();
        let capture_ptr = sck_wrapper::start_capture(
            sck_config,
            make_frame_callback(frame_sender.clone()),
            make_audio_callback(audio_sender.clone()),
        )?;
