...
use super::{VideoFormat, VideoFrameData};
...
impl ProjectLoader {
    ...
    pub async fn load_project(&mut self, project: &Project) -> Result<()> {
        ...
        println!(
            "Project loaded: {} video decoders, {} audio decoders, {} cursor events",
    ...
    pub fn get_cursor_events(
        &self,
        start_ms: u64,
        end_ms: u64,
    ) -> Vec<crate::capture::CursorEvent> {
        self.cursor_events
            .iter()
    ...
    async fn load_cursor_events(&mut self, project: &Project) -> Result<()> {
        for clip in &project.clips {
            ...
            if let Ok(cursor_data) = tokio::fs::read_to_string(&cursor_file).await {
                if let Ok(mut clip_events) =
                    serde_json::from_str::<Vec<crate::capture::CursorEvent>>(&cursor_data)
                {
...
impl VideoDecoder {
    ...
    pub async fn decode_frame_at_time(&self, timestamp_ms: u64) -> Result<Option<VideoFrameData>> {
        ...
        if timestamp_ms < 60000 {
            // Only provide frames for first minute
...
impl AudioDecoder {
    ...
    pub async fn decode_samples_in_range(
        &self,
        _start_ms: u64,
        duration_ms: u64,
    ) -> Result<Option<Vec<f32>>> {
