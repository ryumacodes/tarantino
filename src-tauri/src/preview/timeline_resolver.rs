...
use super::{CursorData, ProjectLoader, ResolvedFrameData, ZoomData};
use crate::capture::{Clip, CursorEvent, Project};
...
impl TimelineResolver {
    ...
    pub async fn resolve_frame(&self, project_time_ms: u64) -> Result<ResolvedFrameData> {
        ...
        if let Some(loader) = project_loader {
            ...
            Err(anyhow::anyhow!(
                "No project loader set in timeline resolver"
            ))
        }
    }
            ...
            for event in &project.cursor_events {
                if event.t <= project_time_ms {
                    if let Some(next_event) =
                        project.cursor_events.iter().find(|e| e.t > project_time_ms)
                    {
