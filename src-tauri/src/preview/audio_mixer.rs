...
impl AudioMixer {
    ...
    pub async fn new(sample_rate: u32) -> Result<Self> {
        ...
        println!(
            "Audio mixer initialized: {}Hz, {} channels",
    ...
    pub async fn start_playback(&mut self, start_time_ms: u64) -> Result<()> {
        ...
        self.audio_command_sender
            .send(AudioCommand::StartPlayback(start_time_ms))
    ...
    pub async fn pause(&mut self) -> Result<()> {
        ...
        if let Some(stream) = &self.stream {
            stream
                .pause()
                .map_err(|e| anyhow::anyhow!("Failed to pause audio stream: {}", e))?;
        }
    ...
    pub async fn seek(&mut self, time_ms: u64) -> Result<()> {
        ...
        self.audio_command_sender
            .send(AudioCommand::Seek(time_ms))
    ...
    pub async fn set_playback_speed(&mut self, speed: f64) -> Result<()> {
        ...
        self.audio_command_sender
            .send(AudioCommand::SetSpeed(speed))
    ...
    async fn process_audio_chunk(
        ...
        if let Some(loader) = loader {
            ...
            let mic_samples = loader_guard
                .get_audio_samples("mic", 0, start_time, chunk_duration_ms)
                .await
                .unwrap_or_default();
            let system_samples = loader_guard
                .get_audio_samples("system", 0, start_time, chunk_duration_ms)
                .await
                .unwrap_or_default();
            drop(loader_guard);
...
fn mix_audio_tracks(
    mic_samples: Option<Vec<f32>>,
    system_samples: Option<Vec<f32>>,
) -> Option<Vec<f32>> {
