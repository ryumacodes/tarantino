...
impl PostProcessor {
    ...
    pub async fn process_video(
        ...
        let zoom_processed_path = if self.config.enable_zoom_effects && zoom_analysis.is_some() {
            ...
            self.apply_zoom_effects(input_path, &temp_path, zoom_analysis.unwrap())
                .await?;
            temp_path
        } else {
        ...
        let final_output = if self.config.enable_cursor_overlay {
            self.apply_cursor_overlay(&zoom_processed_path, output_path)
                .await?;
    ...
    async fn apply_zoom_effects(
        ...
        let filter_complex = self.build_zoom_filter(
            &zoom_analysis.zoom_blocks,
            zoom_analysis.session_duration,
            width,
            height,
        )?;
        ...
        cmd.arg("-i")
            ...
            .arg("medium")
            .arg("-crf")
            .arg("18") // High quality for zoom effects
    ...
    fn build_zoom_filter(
        ...
        for block in zoom_blocks.iter() {
            ...
            let zoom_part = format!(
                ...
                start_time, end_time, block.zoom_factor, start_time, duration
            );
    ...
    async fn apply_cursor_overlay(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        let mouse_events = if sidecar_path.exists() {
            match self.load_mouse_events(&sidecar_path).await {
                ...
                Err(e) => {
                    println!(
                        "Warning: Failed to load mouse events: {}, proceeding without cursor",
                        e
                    );
                    std::fs::copy(input_path, output_path)?;
                    return Ok(());
        ...
        self.create_cursor_overlay_video(input_path, output_path, &mouse_events)
            .await?;
    ...
    fn build_cursor_movement_filter(
        &self,
        mouse_events: &[MouseEvent],
        _cursor_path: &Path,
    ) -> Result<String> {
        ...
        if let Some(first_click) = mouse_events
            .iter()
            .find(|e| matches!(e.event_type, MouseEventType::ButtonPress { .. }))
        {
            let filter = format!(
                "[0:v]overlay={}:{}[output]",
                first_click.x as i32, first_click.y as i32
            );
            Ok(filter)
        } else {
...
mod tests {
    ...
    fn test_zoom_filter_generation() {
        ...
        let filter = processor
            .build_zoom_filter(&zoom_blocks, 5000, 1920, 1080)
            .unwrap();
