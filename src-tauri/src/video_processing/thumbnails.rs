...
pub async fn extract_thumbnails(
    temp_dir: &Path,
    thumbnail_cache: &std::sync::Arc<
        std::sync::RwLock<std::collections::HashMap<(PathBuf, u32, u32), Vec<PathBuf>>>,
    >,
    ...
    for i in 0..thumbnail_count {
        ...
        let task = tokio::spawn(async move {
            ...
            let result = manager_clone
                .execute_operation(thumbnail_operation, OperationPriority::Low)
                .await;
            match result {
    ...
    println!(
        "Generated {} thumbnails for timeline scrubbing (requested: {})",
        thumbnails.len(),
        thumbnail_count
    );
