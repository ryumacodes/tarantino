...
use crate::cursor_renderer::{parse_cursor_events, CursorRenderer};

/// Transform cursor coordinates through zoom. Returns None if cursor is off-screen.
fn transform_cursor_through_zoom(
    x: f64,
    y: f64,
    zoom: f64,
    center_x: f64,
...
pub fn generate_cursor_frames(
    ...
    println!(
        "  Dimensions: {}x{} @ {}fps for {}ms",
        width, height, fps, duration_ms
    );
    println!(
        "  Settings: enabled={:?}, style={:?}",
        cursor_settings.enabled, cursor_settings.style
    );
    ...
    let (events, scale_factor, eff_x, eff_y, eff_w, eff_h) =
    ...
    println!(
        "  Parsing {} raw events, scale_factor={}, area=({},{} {}x{})",
        events.len(),
        scale_factor,
        eff_x,
        eff_y,
    ...
    if let Some(ref trajectory) = zoom_trajectory {
        if !trajectory.is_empty() {
            println!(
                "  Applying spring-physics zoom trajectory ({} frames) to cursor coordinates",
                trajectory.len()
            );
    ...
    let events_duration_ms = cursor_events
        .iter()
    ...
    if effective_duration_ms != duration_ms {
        println!(
            "  Duration corrected: passed={}ms, events_max={}ms, using={}ms",
    ...
    println!(
        "=== CURSOR FRAME GENERATION COMPLETE: {} frames ===",
        frames.len()
    );
...
pub fn export_with_cursor_compositing(
    ...
    println!(
        "  Dimensions: {}x{} @ decoder_fps={}, output_fps={}",
        width, height, decoder_fps, output_fps
    );
    println!("  Cursor frames available: {}", cursor_frames.len());
    println!(
        "  Zoom trajectory: {} frames",
    ...
    println!(
        "=== RUST COMPOSITING COMPLETE: {} frames in {:.1}s ({:.1} fps) ===",
        frame_idx,
        elapsed.as_secs_f64(),
        fps
    );
...
pub fn composite_cursor_frame(
    ...
    if video_frame.len() != expected_size {
        println!(
            "Warning: frame size mismatch: {} vs {}",
            video_frame.len(),
            expected_size
        );
        return;
