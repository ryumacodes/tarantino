...
pub struct VisualSettings {
    ...
    pub window_layout_mode: Option<String>,
...
pub struct CursorSettings {
    ...
    pub loop_to_start: Option<bool>,
    pub loop_duration_ms: Option<u64>,
}
...
pub struct ExportSettings {
    ...
    pub webcam_x: Option<f64>,         // normalized center X (0.0-1.0)
    pub webcam_y: Option<f64>,         // normalized center Y (0.0-1.0)
...
pub fn validate_export_zoom_blocks(blocks: &mut Vec<ZoomBlock>, duration_ms: u64) {
    ...
    println!(
        "Validated {} zoom blocks (duration: {}ms)",
        blocks.len(),
        duration_ms
    );
}
...
pub fn load_zoom_blocks_from_sidecar(video_path: &std::path::Path) -> Option<Vec<ZoomBlock>> {
    ...
    match auto_zoom::load_analysis(&zoom_path) {
        Ok(mut analysis) => {
            ...
            let blocks: Vec<ZoomBlock> = analysis
                .zoom_blocks
                .into_iter()
                .map(|b| ZoomBlock {
