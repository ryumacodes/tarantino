...
use super::types::{CursorSettings, ExportSettings};
...
pub fn get_cursor_config(settings: &ExportSettings) -> CursorSettings {
    settings
        ...
        .unwrap_or_else(|| CursorSettings {
            ...
            stop_at_end: Some(false),
            stop_duration_ms: Some(0),
            loop_to_start: Some(false),
...
pub fn get_webcam_info(input_path: &Path) -> Option<(PathBuf, f64, f64, f64, String)> {
    ...
    let webcam_webm = dir.join(format!("{}.webcam.webm", base_stem));
    if webcam_webm.exists() {
        println!("Found webcam overlay: {}", webcam_webm.display());
        return Some((webcam_webm, 0.85, 0.85, 0.15, "circle".to_string()));
    }
