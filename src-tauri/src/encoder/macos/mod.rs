...
impl VideoToolboxEncoder {
    ...
    pub fn new(config: EncoderConfig) -> Result<Self> {
        ...
        // Create encoder
        let encoder_ptr = unsafe { vt_encoder_create(vt_config, context, Self::frame_callback) };

        if encoder_ptr.is_null() {
            // Clean up context
            unsafe {
                drop(Box::from_raw(
                    context as *mut Arc<Mutex<mpsc::UnboundedSender<EncodedFrame>>>,
                ));
            }
        ...
        println!(
            "VideoToolbox encoder created: {}x{} @ {} fps, {} bps",
    ...
    extern "C" fn frame_callback(context: *mut c_void, frame: VTEncodedFrame) {
        ...
        let sps = if !frame.sps_data.is_null() && frame.sps_len > 0 {
            let sps_slice = unsafe { std::slice::from_raw_parts(frame.sps_data, frame.sps_len) };
        ...
        let pps = if !frame.pps_data.is_null() && frame.pps_len > 0 {
            let pps_slice = unsafe { std::slice::from_raw_parts(frame.pps_data, frame.pps_len) };
        ...
        if let Err(e) = Self::validate_encoded_frame(&encoded_frame) {
            ...
            println!(
                "[ENCODER ERROR] Frame details - size: {}, keyframe: {}, pts: {}, dts: {}",
