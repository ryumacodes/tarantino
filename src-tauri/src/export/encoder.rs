...
use anyhow::Result;
use ffmpeg_next::{self as ffmpeg, codec, format, frame, media, Packet};

use super::{AudioCodec, AudioInfo, ExportConfig, OutputInfo, VideoCodec, VideoInfo};
use crate::export::{AudioTrack, Project};
...
impl VideoEncoder {
    ...
    pub async fn add_audio_track(&mut self, audio_track: &AudioTrack) -> Result<()> {
        println!(
            "Adding audio track: {} ({}Hz, {} channels)",
    ...
    pub async fn finalize(&mut self) -> Result<OutputInfo> {
        ...
        println!(
            "Video encoding completed: {} frames encoded",
            self.encoding_state.frames_encoded
        );
    ...
    fn convert_frame_format(&self, frame_data: &[u8]) -> Result<Vec<u8>> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if rgba_offset + 3 >= frame_data.len() {
                    break;
                }
                ...
                if y % 2 == 0 && x % 2 == 0 {
                    ...
                    if u_offset < yuv_data.len() {
                        yuv_data[u_offset] = u_val;
                    }
                    if v_offset < yuv_data.len() {
                        yuv_data[v_offset] = v_val;
                    }
                }
            }
        }
