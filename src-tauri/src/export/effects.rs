...
impl EffectsProcessor {
    ...
    pub async fn apply_zoom_effects(
        &mut self,
        frame_data: &[u8],
        timestamp_ms: u64,
    ) -> Result<Vec<u8>> {
        ...
        if let Some(active_zoom) = self.get_dominant_zoom() {
            self.apply_zoom_to_frame(frame_data, active_zoom, timestamp_ms)
                .await
        } else {
    ...
    pub async fn apply_cursor_enhancement(&mut self) -> Result<()> {
        ...
        println!(
            "Cursor enhancement configured with settings: {:?}",
            self.cursor_enhancement
        );
        Ok(())
    }
    ...
    fn get_dominant_zoom(&self) -> Option<&ActiveZoomEffect> {
        self.active_zooms
            ...
            .max_by(|a, b| {
                a.zoom_recommendation
                    .confidence
                    .partial_cmp(&b.zoom_recommendation.confidence)
                    .unwrap()
            })
    }
    ...
    async fn zoom_frame(
        ...
        if zoom_factor > 1.01 {
            println!(
                "Applying zoom: {:.2}x at focus ({:.2}, {:.2})",
                zoom_factor, focus_x, focus_y
            );
        }
                        ...
                        for dy in -1i32..=1 {
                            for dx in -1i32..=1 {
                                ...
                                if ny >= 0 && nx >= 0 && ny < height as i32 && nx < width as i32 {
                                    let neighbor_offset =
                                        ((ny as usize * width + nx as usize) * channels + c)
                                            as usize;
        ...
        for angle in 0..360 {
            ...
            if x >= 0 && y >= 0 && (x as u32) < frame.width && (y as u32) < frame.height {
                ...
                if pixel_offset + 3 < frame.data.len() {
                    ...
                    frame.data[pixel_offset] = ((frame.data[pixel_offset] as f32 * (1.0 - alpha))
                        + (highlight_color[0] as f32 * alpha))
                        as u8;
                    frame.data[pixel_offset + 1] =
                        ((frame.data[pixel_offset + 1] as f32 * (1.0 - alpha))
                            + (highlight_color[1] as f32 * alpha)) as u8;
                    frame.data[pixel_offset + 2] =
                        ((frame.data[pixel_offset + 2] as f32 * (1.0 - alpha))
                            + (highlight_color[2] as f32 * alpha)) as u8;
                }
            }
        }
