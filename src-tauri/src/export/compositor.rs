...
use super::{ExportConfig, Project, VideoTrack};
...
impl FrameCompositor {
    ...
    pub async fn initialize(&mut self, project: &Project) -> Result<()> {
        println!(
            "Initializing frame compositor with {} clips",
            project.clips.len()
        );
    ...
    fn add_video_decoder(&mut self, id: &str, track: &VideoTrack) -> Result<()> {
        ...
        println!(
            "Added video decoder for {}: {}x{} @ {}fps",
            id, track.width, track.height, track.fps
        );
    ...
    async fn decode_frame_at_timestamp(
        ...
        let frame = Self::decode_frame_ffmpeg(
            &decoder.video_path,
            timestamp_ms,
            decoder.width,
            decoder.height,
    ...
    async fn decode_frame_ffmpeg(
        ...
        if output.stdout.len() != expected_size {
            ...
            println!(
                "Warning: decoded frame size mismatch, got {} expected {}",
                output.stdout.len(),
                expected_size
            );
    ...
    fn composite_layer(
        ...
        for dst_y in 0..config.size.1 {
            for dst_x in 0..config.size.0 {
                ...
                if out_x < 0
                    || out_y < 0
                    || out_x >= output_width as i32
                    || out_y >= output_height as i32
                {
                    continue;
                }
                ...
                output[dst_idx + 1] =
                    ((src_g as f32 * alpha) + (dst_g as f32 * (1.0 - alpha))) as u8;
                output[dst_idx + 2] =
                    ((src_b as f32 * alpha) + (dst_b as f32 * (1.0 - alpha))) as u8;
