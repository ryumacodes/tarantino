...
impl ZoomPreviewGenerator {
    ...
    pub fn generate_preview_indicators(
        &self,
        mouse_events: &[MouseEvent],
        session_duration: u64,
    ) -> Result<PreviewZoomAnalysis> {
    ...
    fn create_preview_indicators(
        &self,
        click_events: &[&MouseEvent],
        session_duration: u64,
    ) -> Result<Vec<PreviewZoomIndicator>> {
...
#[command]
pub async fn get_preview_zoom_indicators(
    video_path: String,
) -> Result<PreviewZoomAnalysis, String> {
    ...
    let session_duration = mouse_events.iter().map(|e| e.timestamp).max().unwrap_or(0);

    // Generate preview indicators
    let generator = ZoomPreviewGenerator::with_default_config();
    let analysis = generator
        .generate_preview_indicators(&mouse_events, session_duration)
        .map_err(|e| format!("Failed to generate preview indicators: {}", e))?;

    println!(
        "Generated {} preview zoom indicators from {} clicks in {}ms",
...
mod tests {
    ...
    fn test_preview_indicator_generation() {
        ...
        let analysis = generator
            .generate_preview_indicators(&mouse_events, 5000)
            .unwrap();
    ...
    fn test_click_spacing() {
        ...
        let analysis = generator
            .generate_preview_indicators(&close_clicks, 5000)
            .unwrap();
