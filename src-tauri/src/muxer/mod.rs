...
impl Mp4Muxer {
    ...
    fn initialize_track(&mut self) -> Result<()> {
        ...
        println!(
            "Video track initialized: track_id={}, timescale={}",
            track_id, self.timescale
        );
    ...
    fn filter_parameter_sets_from_avcc(&self, data: &[u8]) -> Vec<u8> {
        ...
        while pos + 4 < data.len() {
            ...
            if nal_len > 10_000_000 {
                // 10MB is way too large for a single NAL unit in screen recording
                // Don't log individual NAL issues - aggregate them in summary below
                invalid_nals_skipped += 1;
        ...
        if invalid_nals_skipped > 0 {
            println!(
                "[MUXER WARNING] Skipped {} corrupted/invalid NAL units from AVCC sample",
                invalid_nals_skipped
            );
        }
