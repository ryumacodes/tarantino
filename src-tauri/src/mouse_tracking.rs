...
pub struct KeyEvent {
    ...
    pub is_typing: bool,
    /// Privacy-safe typing movement category. Does not store the typed character.
    #[serde(default)]
    pub key_motion: KeyMotion,
    /// Last known cursor X coordinate when the key was pressed.
    #[serde(default)]
    pub cursor_x: Option<f64>,
    /// Last known cursor Y coordinate when the key was pressed.
    #[serde(default)]
    pub cursor_y: Option<f64>,
    /// Focused text caret X coordinate when available.
    #[serde(default)]
    pub caret_x: Option<f64>,
    /// Focused text caret Y coordinate when available.
    #[serde(default)]
...
mod macos_accessibility {
    ...
    unsafe fn copy_range_attribute(
        element: AXUIElementRef,
        attribute: CFStringRef,
    ) -> Option<CFRange> {
    ...
    fn estimated_caret_from_visible_range(
        ...
        let offset = (selected_range.location - visible_range.location)
            .clamp(0, visible_range.length) as f64;
            ...
            selected_bounds
                ...
                .or_else(|| {
                    ...
                    create_range_value(previous_char_range).and_then(|previous_char_value| {
                        let bounds = copy_bounds_for_range(
                            element,
        ...
        unsafe {
            ...
            for focused in focused_elements {
                ...
                let caret_position = caret_position_for_element(focused_element, &attrs)
                    ...
                    .or_else(|| {
                        caret_position_from_descendants(focused_element, &attrs, &mut visited)
                    });
...
fn key_motion(key: &Key) -> KeyMotion {
    match key {
        Key::Return | Key::KpReturn => KeyMotion::Newline,
        Key::Backspace | Key::Delete => KeyMotion::Backspace,
        Key::Tab => KeyMotion::Tab,
        _ => KeyMotion::Character,
    }
}
...
impl MouseTracker {
    ...
    pub fn start_tracking(&mut self) -> Result<()> {
        println!(
            "🖱️ [TRACKER] start_tracking called, current is_tracking: {}",
            self.is_tracking
        );
        ...
        if let Err(permission_error) = crate::permissions::validate_mouse_tracking_permissions() {
            println!(
                "❌ [TRACKER] Permission validation failed: {}",
                permission_error
            );
            return Err(anyhow::anyhow!(
                "Cannot start mouse tracking: {}",
                permission_error
            ));
        }
    ...
    pub fn add_event(&self, event: MouseEvent) {
        ...
        if current_count == 1 {
            println!(
                "🖱️ [TRACKER] First event captured! Total: {}",
                current_count
            );
        } else if current_count % 500 == 0 {
            println!(
                "🖱️ [TRACKER] Event milestone: {} events captured",
                current_count
            );
        }
        ...
        if is_click {
            println!(
                "🖱️ [TRACKER] CLICK captured at ({:.1}, {:.1}) - Total events: {}",
                event.x, event.y, current_count
            );
        }
    ...
    /// Update the latest typing key with a refined caret position, usually sampled
    /// on key release after the focused app has advanced its insertion point.
    pub fn update_latest_typing_caret(&self, caret_x: f64, caret_y: f64) {
        if !self.is_tracking {
            return;
        }

        let mut key_events = self.key_events.lock();
        if let Some(event) = key_events.iter_mut().rev().find(|event| event.is_typing) {
            event.caret_x = Some(caret_x);
            event.caret_y = Some(caret_y);
        }
    }
        ...
        let callback = move |event: Event| {
            ...
            let mouse_event = match event.event_type {
                ...
                EventType::ButtonPress(button) => {
                    ...
                    println!(
                        "🖱️ Click detected at position: ({:.1}, {:.1}) - Button: {:?}",
                ...
                EventType::KeyPress(key) => {
                    ...
                    let caret_position = if is_typing {
                        macos_accessibility::focused_caret_position()
                    } else {
                        None
                    };
                    tracker_guard.add_key_event(KeyEvent {
                        ...
                        is_typing,
                        key_motion: key_motion(&key),
                        cursor_x: Some(cursor_x),
                        cursor_y: Some(cursor_y),
                        caret_x: caret_position.map(|(x, _)| x),
                        caret_y: caret_position.map(|(_, y)| y),
                    });

                    None // No mouse event
                }
                EventType::KeyRelease(key) => {
                    let (_, is_char_key) = classify_key(&key);
                    if is_char_key && !any_modifier_held() {
                        if let Some((caret_x, caret_y)) =
                            macos_accessibility::focused_caret_position()
                        {
                            tracker_guard.update_latest_typing_caret(caret_x, caret_y);
                        }
                    }
