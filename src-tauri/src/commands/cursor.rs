...
#[tauri::command]
pub async fn cursor_process_event(
    x: f64,
    y: f64,
    timestamp_ms: u64,
) -> Result<Vec<ZoomRecommendation>, String> {
    println!(
        "Processing cursor event at ({}, {}) at {}ms",
        x, y, timestamp_ms
    );

    let mut engine = cursor_engine::create_cursor_engine().map_err(|e| e.to_string())?;

    let event = CursorEvent { x, y, timestamp_ms };
...
pub async fn cursor_update_config(
    ...
    println!(
        "Updating cursor engine config: {}x{}, sensitivity: {}, max zoom: {}",
