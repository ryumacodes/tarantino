...
use std::sync::Arc;
use tauri::{AppHandle, Manager, Position, Size, State};
...
pub async fn show_display_preview(
    ...
    println!(
        "show_display_preview called with display_id: {}",
        display_id
    );

    let mut displays = state.cached_displays();
    if displays.is_empty() {
        displays = state.get_displays().await.map_err(|e| e.to_string())?;
    }
    println!(
        "Available displays: {:?}",
        displays.iter().map(|d| &d.id).collect::<Vec<_>>()
    );
        ...
        match ns_screen_for_cg_display_id(display.cg_display_id) {
            Some((_screen_ptr, frame)) => {
                if frame.size.width <= 0.0 || frame.size.height <= 0.0 {
                    return Err(format!(
                        "Invalid screen dimensions for display {}: {}x{}",
            ...
            None => {
                println!(
                    "Could not find NSScreen for display {}, falling back to primary monitor",
                    display_id
                );
                let monitor = app
                    .primary_monitor()
                ...
                let logical_pos: tauri::LogicalPosition<f64> =
                    physical_pos.to_logical(scale_factor);
    ...
    let (logical_pos, logical_size) = {
        ...
        println!(
            "Monitor info - Scale: {}, Physical: {:?}@{:?}, Logical: {:?}@{:?}",
    ...
    println!(
        "Using full screen rect - Pos: {:?}, Size: {:?}",
        logical_pos, logical_size
    );
    ...
    println!(
        "Preview window positioned and shown at {:?} with size {:?}",
        logical_pos, logical_size
    );
