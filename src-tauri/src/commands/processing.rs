...
async fn wait_for_webcam_sidecar(temp_path: &str, max_wait: tokio::time::Duration) -> bool {
    ...
    loop {
        ...
        if started.elapsed() >= max_wait {
            println!(
                "⚠️ [BG_PROCESS] Timed out waiting for webcam sidecar next to {}",
                temp_path
            );
            return false;
        }
...
pub async fn open_editor_with_loading(
    app: &tauri::AppHandle,
    temp_path: &str,
    has_webcam: bool,
    webcam_shape: &str,
    ...
    tokio::spawn(async move {
        ...
        match state.wait_for_recording_completion().await {
            Ok(_path) => {
                ...
                let mut has_webcam_for_editor = has_webcam;
                if has_webcam {
                    let _ = update_editor_status(&app, "Saving webcam...").await;
                    has_webcam_for_editor =
                        wait_for_webcam_sidecar(&temp_path, tokio::time::Duration::from_secs(15))
                            .await;
                }

                let _ = update_editor_status(&app, "Processing video file...").await;
                println!(
                    "📹 [BG_PROCESS] Processing video file with {} mouse events",
                    mouse_events.len()
                );
                ...
                .await
                {
                    ...
                    let _ = update_editor_status(&app, "Error: Recording failed to finalize").await;
                }

                let _ = app.emit("recording-stopped", final_path);
            }
            Err(e) => {
...
pub async fn process_recorded_file(
    ...
    println!("📁 [PROCESS] Key events received: {}", key_events.len());

    // Count click events specifically
    let click_count = mouse_events
        .iter()
        .filter(|e| {
            matches!(
                e.event_type,
    ...
    if mouse_events.is_empty() {
        println!(
            "⚠️ [PROCESS] WARNING: No mouse events received! Mouse tracking may not be working."
        );
    }

    if key_events.is_empty() {
        println!("⌨️ [PROCESS] No key events received; typing zoom will not be generated.");
    }
    ...
    let start_ms = start_time
        ...
        .as_millis() as u64;
    println!(
        "⏱️ [PROCESS] Recording start time (ms since epoch): {}",
        start_ms
    );
    ...
    if !std::path::Path::new(&auto_zoom_path).exists() {
        ...
        match processor.analyze_session(&session, &normalized_key_events) {
            Ok(analysis) => {
    ...
    println!(
        "✅ [PROCESS] Processing complete. Final path: {}",
        media_path
    );
    Ok(media_path)
}
