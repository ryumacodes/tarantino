...
pub async fn record_stop(
    ...
    println!(
        "✅ [RECORD_STOP] Collected {} mouse events",
        mouse_events.len()
    );
    ...
    let has_webcam = has_webcam_before_release;
    ...
    crate::commands::processing::open_editor(
        &app,
        &final_media_path,
        &sidecar_path,
        has_webcam,
...
pub async fn record_stop_instant(
    ...
    match stop_result {
        Ok((temp_path, recording_start_time)) => {
            ...
            println!(
                "🖱️ [STOP_INSTANT] Captured recording_start_time: {:?}",
                recording_start_time
            );
            if has_webcam {
